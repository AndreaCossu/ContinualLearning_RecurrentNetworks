...
def mnist_exp(args):
    ...
    dataset = clutils.datasets.CLMNIST(args.dataroot, download=False, pixel_in_input=args.pixel_in_input, perc_test=0.25,
        ...
        normalization=None, max_label_value=args.max_label_value, len_task_vector=args.n_tasks if args.agem else 0,
    ...
    if args.lwf:
        n_classes_per_task = 2 if args.split else 10
        args.lwf_alpha = args.lwf_alpha[0] if isinstance(args.lwf_alpha, (list, tuple)) and len(args.lwf_alpha) == 1 else args.lwf_alpha
        # args.lwf_alpha = [0, 1/2, 2*(2/3), 3*(3/4), 4*(4/5)] if args.split else 1
        lwf = LWF(model, device, classes_per_task=n_classes_per_task, alpha=args.lwf_alpha,
            ...
            for epoch in range(1, args.epochs+1):
                ...
                for id_batch, (x,y) in enumerate(loader_train):
                    ...
                    if args.ewc_lambda > 0:
                        ...
                        training_loss, training_accuracy = trainer.train_mas(x, y, mas, task_id, args.truncated_time)
                    elif args.lwf:
...
def mnist_multitask_exp(args):
    ...
    if args.split:
        dataset = clutils.datasets.MMNIST(args.dataroot, download=False, pixel_in_input=args.pixel_in_input, perc_val=0.15,
            train_batch_size=args.batch_size, test_batch_size=512, sequential=args.sequential,
            return_sequences=model.is_recurrent, split=True)
    else:
        dataset = clutils.datasets.MMNIST(args.dataroot, download=False, pixel_in_input=args.pixel_in_input, perc_val=0.15,
            train_batch_size=args.batch_size, test_batch_size=512, return_sequences=model.is_recurrent, split=False)

    loader_train, loader_val, loader_test = dataset.get_loaders()
